...
public class DragableUI : UIBehaviour, IBeginDragHandler, IDragHandler
{
    ...
    private RectTransform dragObjectInternal
    {
        get
        {
            if (null == dragObject) dragObject = transform as RectTransform;
            return dragObject;
        }
    }
    ...
    private RectTransform dragAreaInternal
    {
        get
        {
            if (null == dragArea)
            {
                Canvas[] c = transform.GetComponentsInParent<Canvas>();
                dragArea = (c.Length > 0 ? c[0].transform : transform) as RectTransform;
            }
            return dragArea;
        }
    }
